...
func (rH RouterHandler) articlePost(c *gin.Context) {
	...
	article, err := rH.ucHandler.ArticlePost(articleFromReq(req))
	if err != nil {
		log(err)
...
func (rH RouterHandler) articlePut(c *gin.Context) {
	...
	article, err := rH.ucHandler.ArticlePut(c.Param("slug"), articleFromReq(req))
	if err != nil {
		log(err)
		c.Status(http.StatusUnprocessableEntity)
		return
...
func (rH RouterHandler) articleDelete(c *gin.Context) {
	...
	if err := rH.ucHandler.ArticleDelete(c.Param("slug")); err != nil {
		log(err)
		c.Status(http.StatusUnprocessableEntity)
		return
	}
